...
宠物助手当前是一个三端联调项目，并已补齐轻量商城闭环：

- 用户端：`frontend`
- 管理端：`admin-web`
- 后端：`backend`

项目默认以“本地开发可直接演示”为目标，开发环境启动后会自动补齐演示数据，不依赖额外数据库服务。当前除了宠物档案、健康、护理、AI、社区、知识外，还支持用品推荐关联商品、直接下单、模拟支付、取消未支付订单，以及商家在现有后台内管理自己的商品和订单。
...
- `merchant_demo_1 / merchant123456`
  - 归属商家：`宠物生活馆`
- `merchant_demo_2 / merchant123456`
  - 归属商家：`健康护理实验室`
...
1. 用户端打开“知识”或用品推荐，进入商品详情页
2. 选择规格后立即下单
3. 在地址页新增或选择地址，提交订单
4. 在“我的订单”里执行模拟支付或取消未支付订单
5. 商家用 `merchant_demo_1` 或 `merchant_demo_2` 登录后台
6. 进入商品管理查看自己的商品
7. 进入订单管理处理自己的订单状态

当前范围说明：

- 暂不接真实微信/支付宝支付，使用“模拟支付”完成闭环
- 暂不做购物车、优惠券、退款、运费模板、结算
- 一个订单只包含单一商家的商品
...
```bash
npm run check:all
npm --prefix backend run test:e2e -- --runInBand shop.e2e-spec.ts
```

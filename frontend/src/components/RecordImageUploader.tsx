...
import { healthApi, resolveMediaUrl } from '../services/api';
...
const RecordImageUploader = ({
  ...
  onChange,
  label = '凭证图片',
  helperText = '支持拍照或从相册选择，用于保存疫苗本、体检单等记录。',
  ...
  const handleUploadAsset = async (uri?: string) => {
    ...
    setUploading(true);
    try {
      const response = await healthApi.uploadRecordImage(uri);
    ...
    <View style={styles.container}>
      ...
      {value ? (
        ...
        <View style={styles.placeholder}>
          <Text style={styles.placeholderText}>上传后可在健康记录中直接查看</Text>
        </View>
      )}
    </View>
  );
